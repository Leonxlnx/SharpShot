...
if (-not $sourceText.Contains("AssemblyVersion(`"$version.0`")") -or
    ...
    -not $sourceText.Contains("SharpShot Quick $version\n\n") -or
...
Copy-Item -LiteralPath "$repoRoot\Install.ps1" -Destination "$portableRoot\Install.ps1"
Copy-Item -LiteralPath "$repoRoot\Install.cmd" -Destination "$portableRoot\Install.cmd"
Copy-Item -LiteralPath "$repoRoot\Uninstall.ps1" -Destination "$portableRoot\Uninstall.ps1"
...
$expectedNames = @('Install.cmd', 'Install.ps1', 'LICENSE', 'README.md', 'SHA256SUMS.txt', 'SharpShot.exe', 'SharpShot.exe.config', 'Uninstall.ps1')
